...
if ( ! class_exists( 'dd_recaptcha_class' ) ) {
	class dd_recaptcha_class {
		...
		function actions_filters() {
			if ( dd_recaptcha_is_form_enabled( 'fep_contact_form' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'login' ) && ! defined( 'XMLRPC_REQUEST' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'wc_checkout' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'registration' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'bp_register' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'ms_user_signup' ) && is_multisite() ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'lost_password' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'reset_password' ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'comment' ) && ( ! is_admin() || ! current_user_can( 'moderate_comments' ) ) ) {
			...
			if ( dd_recaptcha_is_form_enabled( 'bbp_reply' ) ) {
		...
		function captcha_form_field() {
			self::$captcha_count++;
			$no_js    = dd_recaptcha_get_option( 'no_js' );
			$site_key = trim( dd_recaptcha_get_option( 'site_key' ) );
			$number   = $this->total_captcha();
			$version = dd_recaptcha_get_option( 'captcha_version', 'v2_checkbox' );

			$field = '<div class="dd_recaptcha_captcha_field"><div id="dd_recaptcha_captcha_field_' . $number . '" class="dd_recaptcha_captcha_field_div">';
			if ( 'v3' === $version ) {
		...
		function footer_script() {
			...
			if ( ! $number && ( 'v3' !== $version || 'all_pages' !== dd_recaptcha_get_option( 'v3_script_load', 'all_pages' ) ) ) {
				return;
			}
		...
		function v2_checkbox_script() {
			...
			<script type="text/javascript">
				var dd_recaptcha_onloadCallback = function() {
					for ( var i = 0; i < document.forms.length; i++ ) {
						var form = document.forms[i];
						var captcha_div = form.querySelector( '.dd_recaptcha_captcha_field_div' );

						if ( null === captcha_div )
							continue;
						captcha_div.innerHTML = '';
						( function( form ) {
							var dd_recaptcha_captcha = grecaptcha.render( captcha_div,{
								'sitekey' : '<?php echo esc_js( trim( dd_recaptcha_get_option( 'site_key' ) ) ); ?>',
								'size'  : '<?php echo esc_js( dd_recaptcha_get_option( 'size', 'normal' ) ); ?>',
								'theme' : '<?php echo esc_js( dd_recaptcha_get_option( 'theme', 'light' ) ); ?>'
							});
							if ( typeof jQuery !== 'undefined' ) {
								jQuery( document.body ).on( 'checkout_error', function(){
									grecaptcha.reset(dd_recaptcha_captcha);
								});
							}
			...
			<?php
			$language = trim( dd_recaptcha_get_option( 'language' ) );

			$lang = '';
			if ( $language ) {
				$lang = '&hl=' . $language;
			}
			$google_url = apply_filters( 'dd_recaptcha_v2_checkbox_script_api_src', 'https://www.google.com/recaptcha/api.js?onload=dd_recaptcha_onloadCallback&render=explicit' . $lang, $lang );
			?>
		...
		function v2_invisible_script() {
					...
					for ( var i = 0; i < document.forms.length; i++ ) {
						...
						( function( form ) {
							var dd_recaptcha_captcha = grecaptcha.render( captcha_div,{
								...
								'size'  : 'invisible',
								'theme' : '<?php echo esc_js( dd_recaptcha_get_option( 'theme', 'light' ) ); ?>',
								'badge' : '<?php echo esc_js( dd_recaptcha_get_option( 'badge', 'bottomright' ) ); ?>',
							...
							if( null !== cf7_submit && ( typeof jQuery !== 'undefined' ) ){
								jQuery( cf7_submit ).off('click').on('click', function( e ){
									...
									grecaptcha.execute( dd_recaptcha_captcha );
								});
							} else {
			...
			$google_url = apply_filters( 'dd_recaptcha_v2_invisible_script_api_src', 'https://www.google.com/recaptcha/api.js?onload=dd_recaptcha_onloadCallback&render=explicit' . $lang, $lang );
			?>
			<script src="<?php echo esc_url( $google_url ); ?>"
				async defer>
			</script>
		...
		function v3_script() {
			...
			$google_url = apply_filters( 'dd_recaptcha_v3_script_api_src', 'https://www.google.com/recaptcha/api.js?render=' . $site_key . $lang, $site_key, $lang );
			?>
			...
			<script type="text/javascript">
				( function( grecaptcha ) {
					...
					grecaptcha.ready( dd_recaptcha_onloadCallback );

					document.addEventListener( 'wpcf7submit', dd_recaptcha_onloadCallback, false );
					if ( typeof jQuery !== 'undefined' ) {
						//Woocommerce
						jQuery( document.body ).on( 'checkout_error', dd_recaptcha_onloadCallback );
					}
					//token is valid for 2 minutes, So get new token every after 1 minutes 50 seconds
					setInterval(dd_recaptcha_onloadCallback, 110000);
		...
		function form_field_return( $return = '' ) {
			if ( is_user_logged_in() && dd_recaptcha_get_option( 'loggedin_hide' ) ) {
				return $return;
			}
			$ip = $_SERVER['REMOTE_ADDR'];
			if ( in_array( $ip, array_filter( explode( '\n', dd_recaptcha_get_option( 'whitelisted_ips' ) ) ) ) ) {
				return $return;
			}
		...
		function post_id() {
			...
			if ( ! absint( dd_recaptcha_get_option( 'failed_login_allow' ) ) ) {
				return 0;
			}
			...
			if ( ! $post_id ) {
				$wpdb->insert( $wpdb->posts, array( 'post_type' => 'ddr-post' ) );
		...
		function show_login_captcha() {
			...
			$count   = absint( dd_recaptcha_get_option( 'failed_login_allow' ) );
		...
		function wc_form_field() {
			if ( ! is_user_logged_in() && 'yes' == get_option( 'woocommerce_enable_signup_and_login_from_checkout', 'yes' ) && dd_recaptcha_is_form_enabled( 'registration' ) ) {
				$this->form_field();

			} elseif ( dd_recaptcha_is_form_enabled( 'wc_checkout' ) ) {
				$this->form_field();
			}
		...
		function ms_form_field( $errors ) {
			if ( $errmsg = $errors->get_error_message( 'dd_recaptcha_error' ) ) {
		...
		function verify( $response = false ) {
			...
			$secre_key  = trim( dd_recaptcha_get_option( 'secret_key' ) );
			...
			$pre_check = apply_filters( 'dd_recaptcha_verify_captcha_pre', null, $response );
			...
			$url = apply_filters( 'dd_recaptcha_google_verify_url', 'https://www.google.com/recaptcha/api/siteverify' );
			...
			if ( isset( $result['success'] ) && true == $result['success'] ) {
				if ( 'v3' === dd_recaptcha_get_option( 'captcha_version' ) ) {
					$score = isset( $result['score'] ) ? $result['score'] : 0;
					$action = isset( $result['action'] ) ? $result['action'] : '';
					$verify = dd_recaptcha_get_option( 'score', '0.5' ) <= $score && 'advanced_nocaptcha_recaptcha' == $action;
				} else {
					$verify = true;
				}
			}
			$verify = apply_filters( 'dd_recaptcha_verify_captcha', $verify, $result, $response );
			$last_verify = $verify;
		...
		function fepcf_verify( $errors ) {
			if ( ! $this->verify() ) {
				$errors->add( 'dd_recaptcha_error', dd_recaptcha_get_option( 'error_message' ) );
			}
		}
		...
		function login_verify( $user, $username = '', $password = '' ) {
			...
			if ( $show_captcha && ! $this->verify() ) {
				return new WP_Error( 'dd_recaptcha_error', $this->add_error_to_mgs() );
			}
		...
		function registration_verify( $errors, $sanitized_user_login, $user_email ) {
			if ( ! $this->verify() ) {
				$errors->add( 'dd_recaptcha_error', $this->add_error_to_mgs() );
			}
		...
		function wc_registration_verify( $errors, $sanitized_user_login, $user_email ) {
			if ( defined( 'WOOCOMMERCE_CHECKOUT' ) && ! dd_recaptcha_is_form_enabled( 'wc_checkout' ) ) {
				return $errors;
			}
			if ( ! $this->verify() ) {
				$errors->add( 'dd_recaptcha_error', dd_recaptcha_get_option( 'error_message' ) );
		...
		function bp_form_field() {
			do_action( 'bp_dd_recaptcha_error_errors' );
		...
		function bp_registration_verify() {
			if ( ! $this->verify() ) {
				buddypress()->signup->errors['dd_recaptcha_error'] = dd_recaptcha_get_option( 'error_message' );
			}
		}

		function ms_form_field_verify( $result ) {
			if ( isset( $_POST['stage'] ) && 'validate-user-signup' === $_POST['stage'] && ! $this->verify() ) {
				$result['errors']->add( 'dd_recaptcha_error', dd_recaptcha_get_option( 'error_message' ) );
			}
		...
		function comment_verify_490( $approved ) {
			if ( ! $this->verify() ) {
				return new WP_Error( 'dd_recaptcha_error', $this->add_error_to_mgs(), 403 );
			}
			return $approved;
		}
		...
		function wpcf7_verify( $result, $tag ) {
			if ( ! $this->verify() ) {
				$result->invalidate( $tag, dd_recaptcha_get_option( 'error_message' ) );
			}
		...
		function bbp_new_verify( $forum_id ) {
			if ( ! $this->verify() ) {
				bbp_add_error( 'dd_recaptcha_error', $this->add_error_to_mgs() );
			}
		}
		...
		function wc_checkout_verify( $data, $errors ) {
			...
			if ( ! is_user_logged_in() && $is_reg_enable && dd_recaptcha_is_form_enabled( 'registration' ) && ( $is_reg_required || ! empty( $data['createaccount'] ) ) ) {
...
add_action( 'init', array( dd_recaptcha_class::init(), 'actions_filters' ), -9 );
